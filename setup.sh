#!/bin/bash
if [ "${EUID}" -ne 0 ]; then
		echo "คุณต้องรันสคริปต์นี้ในฐานะรูท"
		exit 1
fi
if [ "$(systemd-detect-virt)" == "openvz" ]; then
		echo "ไม่รองรับ OpenVZ"
		exit 1
fi
# ==========================================
# Color
...
# Install SlowDNS
wget https://raw.githubusercontent.com/shwelu2024/auto-script/main/SLDNS/install-sldns && chmod +x install-sldns && ./install-sldns
...
echo "1.2" > /home/ver
echo " "
echo "การติดตั้งเสร็จสมบูรณ์!!"echo " "
...
echo ""  | tee -a log-install.txt
echo "   >>> ข้อมูลเซิร์ฟเวอร์และคุณสมบัติอื่น ๆ"  | tee -a log-install.txt
